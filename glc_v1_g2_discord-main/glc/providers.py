...
# ────────────────────────────────────────────────────────────────────────────
# Databricks (Mosaic AI Model Serving — OpenAI-compatible route)
# ────────────────────────────────────────────────────────────────────────────
#
# Databricks serving endpoints expose a unified OpenAI-compatible surface at
#   {workspace_host}/serving-endpoints/chat/completions
# where the request body's `model` field is the serving-endpoint name (e.g.
# "databricks-claude-opus-4-8"). One DatabricksProvider instance is registered
# per endpoint so the capability-aware router can fail over between models the
# same way it did across external providers in V9. They share a single
# workspace host + PAT (DATABRICKS_HOST / DATABRICKS_API_KEY).


class DatabricksProvider(OpenAICompatProvider):
    name = "databricks"
    # Reasoning is resolved per-model in model_capabilities(); default off.
    capabilities = {**OpenAICompatProvider.capabilities, "reasoning": False}

    def __init__(self, api_key, model, host):
        base = host.rstrip("/") + "/serving-endpoints"
        super().__init__(api_key, model, base)
        # Per-endpoint identity so router logs / ProviderError messages name the
        # exact model rather than a generic "databricks".
        self.name = model


# Worker pool — every Databricks serving endpoint the gateway can route to.
# Order here is the catalogue; the active failover order lives in chat.ORDER /
# chat.TIER_TO_ORDER and the rate budgets in routing.LIMITS.
DATABRICKS_MODELS = [
...
# Router pool — small/fast endpoints used only for the TINY/LARGE/HUGE routing
# classifier. They only ever emit a single word, so cheap models suffice.
DATABRICKS_ROUTER_MODELS = [
    "databricks-meta-llama-3-1-8b-instruct",
    "databricks-gemma-3-12b",
    "databricks-gpt-oss-20b",
    "databricks-claude-haiku-4-5",
...
def model_capabilities(provider_name: str, model: str, default_caps: dict) -> dict:
    ...
    # Databricks: provider_name is the serving-endpoint name; resolve reasoning
    # from the model hint (gpt-oss / qwen3 endpoints support reasoning_effort).
    if provider_name.startswith("databricks-"):
        caps["reasoning"] = _model_supports_reasoning(model)
...
def build_providers(cache_store):
    ...
    S11: primary worker pool is now Databricks Mosaic AI Model Serving. Each
    serving endpoint in DATABRICKS_MODELS is registered as its own routable
    provider (sharing DATABRICKS_HOST + DATABRICKS_API_KEY) so the
    capability-aware router fails over between models. The legacy external
    providers below still load if their own keys are present, but are dormant
    when only Databricks credentials are configured.
    """
    out = {}
    host = os.getenv("DATABRICKS_HOST")
    dbx_key = os.getenv("DATABRICKS_API_KEY")
    if host and dbx_key:
        for endpoint in DATABRICKS_MODELS:
            out[endpoint] = DatabricksProvider(dbx_key, endpoint, host)
...
def build_router_providers():
    ...
    dbx_key = os.getenv("DATABRICKS_API_KEY")
    if host and dbx_key:
        for endpoint in DATABRICKS_ROUTER_MODELS:
            out[endpoint] = DatabricksProvider(dbx_key, endpoint, host)
    if k := os.getenv("CEREBRAS_API_KEY"):
