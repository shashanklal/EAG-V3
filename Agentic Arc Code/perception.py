...
def observe(
    ...
    for event in history[-10:]:
        if event.get("kind") == "answer":
            user_parts.append(f"  [iter {event.get('iter')}] ANSWER for goal {event.get('goal_id')}: {event.get('text', '')[:200]}")
        elif event.get("kind") == "action":
            user_parts.append(f"  [iter {event.get('iter')}] ACTION for goal {event.get('goal_id')}: {event.get('tool')}({event.get('arguments', {})}) → {event.get('result_descriptor', '')[:100]}")
    ...
    import re as _re
    SYNTHESIS_VERBS = {"determine", "choose", "recommend", "analyze", "compare", "evaluate", "which", "select", "decide", "identify", "summarize", "explain", "tell", "give", "list", "describe", "based"}
        ...
        for event in history:
            if event.get("goal_id") == g.id:
                if event.get("kind") == "action":
                    ...
                    # so Decision can answer from general knowledge on next iteration
                    if rd.startswith("EXHAUSTED:"):
                        continue
                    success_action_count += 1
                    if success_action_count >= required_actions:
                        has_evidence = True
                        break
