...
Usage:
    ...
    python agent.py --clean A          # clean state before running
    python agent.py --no-clean C2      # preserve state (e.g., after C1)
...
from schemas import Goal, HistoryEvent

MAX_ITERATIONS = 10

# Queries that MUST preserve state from a prior run (no auto-clean)
SEQUENTIAL_QUERIES = {"C2"}  # C2 depends on C1's memory
...
def clean_state():
    """Remove state/ and sandbox/ directories, then recreate empty structure."""
    import time
    for d in (STATE_DIR, SANDBOX_DIR):
        if d.exists():
            # Windows/OneDrive may hold locks — retry with on_error handler
    ...
    print("[clean] state/ and sandbox/ cleared")
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            for it in range(1, MAX_ITERATIONS + 1):
                ...
                if out.answer:
                    ...
                    result = await action.execute(session, out.tool_call, artifacts)
                    print(f"[action]        → {result.descriptor[:100]}")
                    ...
                    history.append(HistoryEvent(
                        ...
                        tool=out.tool_call.name,
                        arguments=out.tool_call.arguments,
                        result_descriptor=result.descriptor[:200],
                        artifact_id=result.artifact_id,
                    ))
            else:
...
def _final_answer(history: list[HistoryEvent]) -> str:
    ...
    answers = [e.text for e in history if e.kind == "answer" and (e.text or "").strip()]
    if not answers:
        # No verbal answers — summarize from actions (file creations, searches, etc.)
        actions = [e for e in history if e.kind == "action"]
        if actions:
            # Try to extract file paths from create_file results
            created_files = []
            for a in actions:
                desc = a.result_descriptor or ""
                # Simple string-based extraction (no regex on LLM output — this is tool output)
                if '"path"' in desc and "ok" in desc:
                    start = desc.find('"path": "')
                    if start >= 0:
                        start += len('"path": "')
                        end = desc.find('"', start)
                        if end > start:
                            created_files.append(desc[start:end])
            if created_files:
...
async def main():
    ...
    force_no_clean = "--no-clean" in args
    args = [a for a in args if a not in ("--clean", "--no-clean")]

    if not args:
        print("Usage: python agent.py [--clean|--no-clean] <A|B|C1|C2|D|\"custom query\">")
        print("  --clean     Force clean state before running")
        print("  --no-clean  Preserve state (skip auto-clean)")
        print("  (default)   Auto-clean unless query is in SEQUENTIAL_QUERIES (C2)")
        print("\nTest Queries:")
    ...
    # Auto-clean decision: clean unless --no-clean or query is sequential (C2)
    if force_clean:
        clean_state()
    elif not force_no_clean:
        if query_key not in SEQUENTIAL_QUERIES:
            clean_state()
        else:
            print(f"[clean] skipped — '{query_key}' depends on prior state")
