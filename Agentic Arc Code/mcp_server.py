...
def _tavily_search(query: str, max_results: int) -> list[dict]:
    """Call Tavily REST API directly via httpx (bypasses SSL issues with their Python SDK)."""
...
async def _crawl4ai_fetch(url: str) -> dict:
    """Fetch URL content using httpx + BeautifulSoup (primary). crawl4ai disabled due to subprocess blocking."""
    return await _httpx_fetch(url)


async def _httpx_fetch(url: str) -> dict:
    """Simple httpx fetch — fetches HTML and strips tags via BeautifulSoup."""
    from bs4 import BeautifulSoup
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}
    async with httpx.AsyncClient(timeout=30, follow_redirects=True, headers=headers, verify=False) as client:
...
def create_file(path: str, content: str) -> dict:
    """Create a new file in the sandbox; errors if it exists. Auto-creates parent directories. Example: create_file("reminders/note.txt", "hi")."""
    p = _safe(path)
    if p.exists():
