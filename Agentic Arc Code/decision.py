...
def next_step(
    ...
    if history:
        ...
        for event in recent:
            if event.get("kind") == "answer":
                user_parts.append(f"  [iter {event.get('iter')}] ANSWER: {event.get('text', '')[:150]}")
            elif event.get("kind") == "action":
                user_parts.append(f"  [iter {event.get('iter')}] ACTION: {event.get('tool')}() → {event.get('result_descriptor', '')[:100]}")
        user_parts.append("")
    ...
    if parsed:
        ...
        if parsed.get("answer"):
            # Check if the "answer" is actually a tool_call JSON (LLM confusion)
            ...
            try:
                ...
                pass
            return DecisionOutput(answer=_clean_answer(ans_text), tool_call=None)
    ...
    try:
        ...
        if data.get("answer"):
            return DecisionOutput(answer=_clean_answer(data["answer"]), tool_call=None)
    except (json.JSONDecodeError, KeyError, TypeError):
        pass

    # Check if raw text contains an embedded tool_call JSON (LLM narrates then outputs JSON)
    tc_match = re.search(r'\{"tool_call":\s*\{[^}]*"name":\s*"([^"]+)"[^}]*"arguments":\s*(\{[^}]*\})', text)
    if tc_match:
        try:
            # Try to extract the full JSON block
            tc_start = text.find('{"tool_call"')
            if tc_start >= 0:
                tc_json = json.loads(text[tc_start:].strip().rstrip('.'))
                if tc_json.get("tool_call"):
                    return DecisionOutput(
                        answer=None,
                        tool_call=ToolCall(
                            name=tc_json["tool_call"]["name"],
                            arguments=tc_json["tool_call"].get("arguments", {}),
                        ),
                    )
        except (json.JSONDecodeError, KeyError, TypeError):
            pass
    ...
    if text.strip():
        return DecisionOutput(answer=_clean_answer(text.strip()), tool_call=None)

    return DecisionOutput(answer="I could not determine the next step.", tool_call=None)


def _clean_answer(text: str) -> str:
    """Strip JSON wrappers, code fences, and meta-prefixes from an answer."""
    import re
    # If the whole text is a tool_call JSON (LLM confusion), discard it
    try:
        maybe_json = json.loads(text.strip())
        if isinstance(maybe_json, dict) and "tool_call" in maybe_json:
            return ""
    ...
    text = re.sub(r'\n?```\s*', '', text)
    # If the whole text is JSON with an "answer" key, extract it
    try:
        ...
        pass
    # If text contains a JSON block with "answer" at the end, extract from it
    ...
    if answer_match:
        try:
            ...
            pass
    # Remove meta-prefixes like "Looking at the Wikipedia content..."
    text = re.sub(r'^(?:Looking at|Based on)[^.]*(?:content|text|information)[^.]*\.\s*', '', text, count=1)
    # Remove option narration prefixes like "<option A - Answer>" or "OPTION A:"
    text = re.sub(r'^\s*<option\s*[AB][^>]*>\s*', '', text, flags=re.IGNORECASE)
    text = re.sub(r'^\s*(?:OPTION\s*[AB]\s*[-–—:]?\s*(?:Answer|Tool call)?\s*[-–—:]?\s*)', '', text, flags=re.IGNORECASE)
    # Remove confidence/note suffixes that are meta-commentary
    text = re.sub(r'\n\s*CONFIDENCE LEVEL:.*$', '', text, flags=re.DOTALL)
    text = re.sub(r'\n\s*Note:.*(?:tool|goal|weather|forecast|information).*$', '', text, flags=re.DOTALL | re.IGNORECASE)
    # Remove trailing tool_call JSON blocks that Decision sometimes appends after an answer
    text = re.sub(r'\n*\s*\{"tool_call":\s*\{.*\}\}\s*$', '', text, flags=re.DOTALL)
    # Remove trailing answer JSON blocks
    text = re.sub(r'\n*\s*\{"answer":\s*".*"\}\s*$', '', text, flags=re.DOTALL)
    return text.strip()
