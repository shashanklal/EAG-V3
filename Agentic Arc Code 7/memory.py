...
def read(
    ...
    *,
    ...
    """Hybrid retrieval: keyword hits first (high precision on exact term
    matches), then vector hits to fill remaining slots. This prevents
    high-volume indexed chunks from drowning out specific stored facts."""
    kw_hits = _keyword_search(query, history, kinds=kinds, top_k=top_k)
    vec_hits = _vector_search(query, kinds=kinds, top_k=top_k)
    # Merge: keyword hits first, then vector hits not already seen.
    seen_ids = {item.id for item in kw_hits}
    merged = list(kw_hits)
...
def remember(
    ...
    *,
    ...
    # Additionally, ALWAYS include `raw` in the value to prevent loss of
    # concrete data (dates, numbers) when the classifier omits them.
    parsed_value = parsed.get("value")
    if not parsed_value:
        parsed_value = {"raw": raw_text}
    elif "raw" not in parsed_value:
        parsed_value["raw"] = raw_text
...
def record_outcome(
    *,
    ...
    if artifact_id:
        ...
        descriptor += result_text[:200].replace("\n", " ")
...
def add_facts_batch(
    ...
    *,
    ...
    """Batch-add facts with concurrent embedding. Each item in fact_specs
    should have keys: descriptor, value. Returns total items persisted.
    Embeddings are computed in parallel via ThreadPoolExecutor."""
    ...
    def _embed_one(spec):
        ...
        emb = _try_embed(desc, task_type="retrieval_document")
        return {**spec, "_embedding": emb}

    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {pool.submit(_embed_one, s): i for i, s in enumerate(fact_specs)}
        for fut in as_completed(futures):
            results.append(fut.result())

    # Phase 2: persist all items (sequential — disk I/O is cheap)
    items_store = _load()
    idx = _index()
    for spec in results:
        desc = spec["descriptor"]
        ...
        item = MemoryItem(
            id=new_id("mem"),
            kind="fact",
            keywords=list({k.lower() for k in list(_tokens(desc))[:10]}),
            descriptor=desc,
            value=val,
            embedding=emb,
        ...
        if emb is not None and item.kind in _EMBEDDABLE_KINDS:
            idx.add(item.id, emb)
    _save(items_store)
    idx.persist()
    return len(results)
