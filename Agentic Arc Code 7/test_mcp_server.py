...
def _clean_sandbox() -> None:
    """Remove only test-created files/dirs, preserving existing project content."""
    # Items that belong to the project and should not be deleted
    _PRESERVE = {"papers", "mom_birthday.txt", "reminder_2weeks_before_mom_birthday.txt", "reminder_mom_birthday.txt"}
    if SANDBOX.exists():
