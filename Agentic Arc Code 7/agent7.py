...
MAX_ITERATIONS_INDEX = 80

INDEX_SIGNALS = ("index every", "index all", "index each", "index the file")


def _max_iterations(query: str) -> int:
    """Return higher iteration limit for bulk-indexing queries."""
    q = query.lower()
    if any(sig in q for sig in INDEX_SIGNALS):
        return MAX_ITERATIONS_INDEX
...
async def run(query: str) -> str:
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            max_iters = _max_iterations(query)
            for it in range(1, max_iters + 1):
                ...
                # 3. DECISION
                out = decision.next_step(goal, hits, attached, history, tools_for_decision, query=query)
                ...
                if out.is_answer:
                    ...
                    # Loop-break: if the same goal got 4+ consecutive
                    # answers that look like stalling (contain tool-call
                    # JSON or are identical repeats), force stop.
                    recent_answers = [
                        h for h in history
                        if h.get("kind") == "answer" and h.get("goal_id") == goal.id
                    ]
                    if len(recent_answers) >= 4:
                        last4 = [a.get("text") or "" for a in recent_answers[-4:]]
                        all_identical = len(set(last4)) == 1
                        all_tool_json = all(
                            '{"tool"' in t or "web_search" in t.lower()
                            for t in last4
                        )
                        if all_identical or all_tool_json:
                            final_answer = (
                                "I was unable to find the requested information in "
                                "the knowledge base."
                            )
                            print(f"[loop-break]    4 consecutive stalled answers — forcing stop")
                            break
