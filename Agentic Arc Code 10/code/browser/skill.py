...
def _enrich_with_ratings(html: str, content: str) -> str:
    """Append star-rating context extracted from CSS classes to trafilatura
    output. Sites like books.toscrape.com encode ratings purely via CSS
    classes (e.g. class="star-rating Five") which trafilatura strips."""
    # Find product containers with star ratings
    # Pattern: star-rating class followed by a book title link
    pattern = _re.compile(
        r'class="star-rating\s+(One|Two|Three|Four|Five)".*?'
        r'<a[^>]*href="([^"]*)"[^>]*>([^<]+)</a>',
    ...
    if not matches:
        return content
    # Also extract prices near each product
    ...
    lines = ["\n\n--- BOOK RATINGS (from page metadata) ---"]
    if price_matches:
        for rating, url, title, price in price_matches:
            lines.append(f"  - {title.strip()} | rating: {rating} | price: {price.strip()} | url: {url.strip()}")
    else:
        for rating, url, title in matches:
            lines.append(f"  - {title.strip()} | rating: {rating} | url: {url.strip()}")
    return content + "\n".join(lines)
...
class BrowserSkill:
    ...
    def __init__(self, *, gateway_url: str = "http://localhost:8109",
                 ...
                 a11y_provider_pin: str | None = "claude_s46",
                 vision_provider_pin: str | None = None,
                 artifacts_root: str | None = None,
                 max_steps_a11y: int = 20,
                 max_steps_vision: int = 20,
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        if html:
            ...
            content = _enrich_with_ratings(html, content)
        ...
        # Report actual turns from the last driver that ran.
        total_turns = (getattr(vis_result, "turns", 0) or len(getattr(vis_result, "steps", []))
                       ) + (getattr(a11y_result, "turns", 0) or len(getattr(a11y_result, "steps", [])))
    ...
    async def _drive(self, DriverCls, url, goal, client, artifacts_dir,
        ...
        async with async_playwright() as p:
            ...
            try:
                # Retry page load + driver run up to 2 extra times on
                # "Execution context destroyed" — the page may redirect or
                # reload during initial evaluation.
                for _load_attempt in range(3):
                    try:
                        await page.goto(url, wait_until="domcontentloaded", timeout=45000)
                        # Wait for any client-side redirects to settle before
                        # evaluating DOM — avoids "Execution context destroyed".
                        ...
                        # Extra settle time — some sites fire late navigations
                        await asyncio.sleep(2.0)
                        # Verify page context is alive before proceeding
                        try:
                        ...
                        if any(m in str(e).lower() for m in _CONTEXT_DESTROYED_MARKERS):
                            if _load_attempt < 2:
                                await asyncio.sleep(2.0)
                                # Create a fresh page for the retry
                                try:
                                    await page.close()
                                except Exception:  # noqa: BLE001
                                    pass
                ...
                try:
                    ...
                    html_content = ""
                kind = detect_gateway_block(html_content)
                if kind:
                    await browser.close()
                    out = DriverResult(
                        success=False,
    ...
    def _pack_error(self, url, goal, code, msg, *, elapsed=0.0, turns=0) -> AgentResult:
        out = BrowserOutput(
            url=url or "", goal=goal, path="extract", turns=turns, content=None,
        )
        return AgentResult(
