...
def classify_failure(error_text: str) -> RecoveryReason:
    ...
    # Coder parse failure — sandbox got no usable code; retrying the whole
    # pipeline is wasteful, treat as validation_error (skip).
    if "no code in upstream coder output" in e:
        return "validation_error"
    # URL index out-of-bounds — upstream researcher had fewer URLs than the
    # planner expected. Re-planning won't help; skip this browser node.
    if "url_from_input: no url at index" in e:
        return "validation_error"
    transient_markers = (
        "503", "502", "504",
        "timeout", "timed out",
