...
() => {
  ...
  for (const el of outermost) {
    ...
    // Context enrichment: look for nearby non-interactive elements that
    // contain useful metadata (star ratings, prices, availability).
    // Walk up to the closest product-card ancestor and extract context.
    let context = '';
    let container = el.closest('article, .product_pod, [class*=product], li, .card, .item');
    if (!container) container = el.parentElement;
    if (container) {
      // Star ratings: books.toscrape.com uses class="star-rating Five" etc.
      const ratingEl = container.querySelector('[class*=star-rating], [class*=rating], [data-rating]');
      if (ratingEl) {
        const cls = ratingEl.className || '';
        const ratingMatch = cls.match(/(?:star-rating|rating)[- ]*(One|Two|Three|Four|Five|\d)/i);
        if (ratingMatch) context += ' [rating:' + ratingMatch[1] + ']';
        const dataR = ratingEl.getAttribute('data-rating');
        if (dataR) context += ' [rating:' + dataR + ']';
      }
      // Price — prefer .price_color (exact price) over broader matches
      const priceEl = container.querySelector('.price_color') ||
                      container.querySelector('.amount, [itemprop=price]');
      if (priceEl) {
        const priceText = (priceEl.textContent || '').trim().split(/\n/)[0].trim().slice(0, 20);
        if (priceText && !name.includes(priceText)) context += ' [price:' + priceText + ']';
      }
      // Availability
      const availEl = container.querySelector('.instock, .availability, [class*=avail]');
      if (availEl) {
        const availText = (availEl.textContent || '').trim().replace(/\s+/g,' ').split(/\n/)[0].trim().slice(0, 20);
        if (availText && availText.length > 2) context += ' [avail:' + availText + ']';
      }
    }
    if (context) name = (name + context).slice(0, 120);
    out.push({
      id: ++id,
