...
async def _dispatch(action: dict, page: Page, snap: PageSnapshot) -> str:
    ...
    if t == "click":
        ...
        # For links, use Promise.all pattern to catch navigation.
        if el.tag == "a":
            try:
                ...
                # TimeoutError means click happened but no navigation
                # (e.g. # anchor, javascript:void). That's fine.
                pass
            # Ensure the page is fully settled after navigation.
            try:
