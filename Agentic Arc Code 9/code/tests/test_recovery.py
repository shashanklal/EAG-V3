...
# Coder parse failure is now classified as validation_error (skip) rather
# than upstream_failure (replan) — retrying the whole pipeline is wasteful.
CODER_VALIDATION_STRINGS = [
...
@pytest.mark.parametrize("err", CODER_VALIDATION_STRINGS)
def test_classify_coder_as_validation(err: str) -> None:
    assert classify_failure(err) == "validation_error", f"misclassified coder error: {err!r}"
