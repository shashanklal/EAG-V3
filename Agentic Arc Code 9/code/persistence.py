...
# NetworkX 3.6+ renamed the `link=` param to `edges=` in node_link_data/graph.
# Detect which API we have so we work on both 3.2-3.5 and 3.6+.
import inspect as _inspect
_nld_sig = _inspect.signature(nx.node_link_data)
_NX_EDGE_KW = "edges" if "edges" in _nld_sig.parameters else "link"
...
def _atomic_write(path: Path, data: bytes | str) -> None:
    ...
    if isinstance(data, bytes):
        ...
        with open(tmp, "w", encoding="utf-8") as f:
            f.write(data)
    # Retry os.replace up to 3 times — OneDrive/AV can hold brief locks on Windows
    import time
...
class SessionStore:
    ...
    def write_graph(self, graph_obj: nx.DiGraph) -> None:
        ...
        for u, v, d in graph_obj.edges(data=True):
            h.add_edge(u, v, **d)
        payload = nx.node_link_data(h, **{_NX_EDGE_KW: "edges"})
    ...
    def read_graph(self) -> nx.DiGraph | None:
        if self.graph_path.exists():
            ...
            g = nx.node_link_graph(payload, **{_NX_EDGE_KW: "edges"}, directed=True)
